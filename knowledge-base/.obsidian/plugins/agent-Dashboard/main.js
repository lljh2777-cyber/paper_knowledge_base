...
var DashboardDataService = class {
    ...
    if (!this.initialized) {
      ...
      for (const change of changes) {
        if (change.type === "delete") {
          nextRecords.delete((0, import_obsidian.normalizePath)(change.path));
          continue;
        }
  ...
  async readRecord(file) {
    ...
    return {
      file,
      path: (0, import_obsidian.normalizePath)(file.path),
      name: file.basename,
      text,
      frontmatter,
  ...
  inferType(path3) {
    const normalized = (0, import_obsidian.normalizePath)(path3);
  ...
  computeInboundReferenceCounts(records) {
    ...
    for (const record of records) {
      for (const match of record.text.matchAll(/\[\[([^\]|#]+)(?:#[^\]|]+)?(?:\|[^\]]+)?\]\]/g)) {
        const target = (0, import_obsidian.normalizePath)(match[1]).replace(/\.md$/i, "");
  ...
  computeLinkReport(records) {
    ...
    for (const record of records) {
      ...
      for (const match of text.matchAll(/\[\[([^\]|#]+)(?:[|#][^\]]*)?\]\]/g)) {
        ...
        const candidates = [
          (0, import_obsidian.normalizePath)(target),
          (0, import_obsidian.normalizePath)(`wiki/${target}`)
        ];
...
function profileHasConfiguredQwenWebSearch(profile) {
  const source = asRecord2(profile);
  const webSearch = asRecord2(source.webSearch);
...
function profileSupportsDirectWebSearch(profile) {
  const source = asRecord2(profile);
  return profileHasConfiguredQwenWebSearch(source) && asRecord2(source.lastTest).webSearchVerified === true;
}
...
function profileSupportsQueryImage(profile) {
  const source = asRecord2(profile);
  return source.type === "openai-compatible" && asRecord2(source.capabilities).vision === true;
}
...
function normalizeProviderProfile(profile) {
  const source = asRecord2(profile);
  const capabilities = asRecord2(source.capabilities);
  const webSearch = asRecord2(source.webSearch);
  const rawLastTest = asRecord2(source.lastTest);
...
var MANAGED_CODEX_BIN_ROOT = path2.join(process.env.LOCALAPPDATA || "", "OpenAI", "Codex", "bin");
...
function isManagedCodexExecutable(executable) {
  ...
  const normalized = path2.resolve(executable).toLowerCase();
...
var ActionInputModal = class extends import_obsidian2.Modal {
...
var TaskResultModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    copy.addEventListener("click", async () => {
      ...
      new import_obsidian2.Notice("任务结果已复制");
    });
    if (this.canRepair()) {
...
var CodePracticeView = class extends import_obsidian2.ItemView {
  ...
  renderRuntime(parent) {
    ...
    (0, import_obsidian2.setIcon)(warning.createSpan(), "shield-alert");
    warning.createSpan({
  ...
  renderNotebook(parent) {
    ...
    (0, import_obsidian2.setIcon)(addFooter, "plus");
  ...
  renderFigures(parent, figures) {
    ...
    figures.forEach((figurePath) => {
      ...
      if (dataUrl) item.createEl("img", { attr: { src: dataUrl, alt: path2.basename(figurePath) } });
  ...
  clearAllCellCode() {
    ...
    this.render();
    new import_obsidian2.Notice("已清空当前语言的代码和输出");
  }
  resetCellsToTwo() {
    if (this.activeRunId) return;
    this.cellsByLanguage[this.language] = this.createDefaultCells(this.language);
    this.render();
    new import_obsidian2.Notice("已重置为两个空单元格");
  }
  addCell(afterIndex) {
    if (this.activeRunId) return;
  ...
  async runCell(cellId, focusNext = false) {
    ...
    if (!code.trim()) {
      new import_obsidian2.Notice("请输入代码");
      return null;
    }
...
${candidate.code.trimEnd()}`).join("\n\n");
  ...
  stopCode() {
    ...
    new import_obsidian2.Notice("正在停止代码练习");
    this.render();
  }
  openSaveModal() {
    ...
    new PracticeNoteModal(this.app, defaultTitle, async (form) => {
      try {
        ...
        new import_obsidian2.Notice(`已保存：${file.path}`);
        await this.app.workspace.getLeaf(true).openFile(file);
      } catch (error) {
        new import_obsidian2.Notice(`保存失败：${error instanceof Error ? error.message : String(error)}`, 8e3);
      }
    }).open();
  }
  displayStatus(status) {
...
var VaultImagePickerModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    const files = this.app.vault.getFiles().filter((file) => Boolean(VAULT_IMAGE_MIME_TYPES[path2.extname(file.path).toLowerCase()])).filter((file) => Number(file.stat?.size || 0) <= MAX_VAULT_IMAGE_BYTES).filter((file) => !this.selectedPaths.has(file.path.toLocaleLowerCase())).sort((a, b) => Number(b.stat?.mtime || 0) - Number(a.stat?.mtime || 0));
    ...
    const renderList = () => {
      ...
      for (const item of visible) {
        ...
        if (references.length) {
          ...
          (0, import_obsidian2.setIcon)(referenceIcon, "file-text");
...
var QueryWikiView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    this.plugin = plugin;
  ...
  async renderMessage(parent, message) {
    ...
    (0, import_obsidian2.setIcon)(icon, message.role === "user" ? "user" : "library-big");
    ...
    if (message.content) {
      ...
      copyButton.addEventListener("click", async () => {
        try {
          ...
          (0, import_obsidian2.setIcon)(copyButton, "check");
          ...
          new import_obsidian2.Notice(`复制失败：${error instanceof Error ? error.message : String(error)}`);
        }
      });
    }
    ...
    if (["pending", "stopping"].includes(message.status)) {
      ...
      await import_obsidian2.MarkdownRenderer.render(this.app, message.content, markdown, "", this);
    }
  ...
  renderRetrievalTrace(parent, trace) {
    ...
    (0, import_obsidian2.setIcon)(summaryIcon, "git-fork");
    summary.createSpan({
  ...
  renderSourcePanel(parent, message) {
    ...
    (0, import_obsidian2.setIcon)(summaryIcon, webSources.length ? "globe-2" : "library-big");
    const summaryParts = [];
  ...
  renderComposer(parent) {
    ...
    (0, import_obsidian2.setIcon)(send, "arrow-up");
    ...
    [
      ...
      button.addEventListener("click", async () => {
        ...
        if (value === "web" && activeBackendId !== "codex-cli" && !profileSupportsDirectWebSearch(activeProfile)) {
          ...
          new import_obsidian2.Notice("当前 Direct API 未通过联网搜索测试；已切换到 Codex CLI");
        }
  ...
  renderExecutionSettings(parent) {
    ...
    (0, import_obsidian2.setIcon)(icon, "sliders-horizontal");
    ...
    backend.addEventListener("change", async () => {
      ...
      if (this.pendingImages.length && !profileSupportsQueryImage(selectedProfile)) {
        this.pendingImages = [];
        new import_obsidian2.Notice("所选后端未启用视觉输入，已移除待发送图片");
      }
      ...
      if (backend.value !== "codex-cli" && this.session.retrievalMode === "web" && !profileSupportsDirectWebSearch(selectedProfile)) {
        ...
        new import_obsidian2.Notice("所选 Direct API 未通过联网搜索测试；已切换为知识库模式");
      }
      await this.render();
      this.inputEl?.focus();
    });
  ...
  async submitQuestion(question) {
    ...
    if (backendId !== "codex-cli" && !directProfile) {
      new import_obsidian2.Notice("所选 Direct API 配置不可用，请重新选择执行后端");
      return;
    }
    ...
    if (selectedImages.length && !profileSupportsQueryImage(directProfile)) {
      new import_obsidian2.Notice("当前执行后端未启用视觉输入，无法发送图片");
      return;
    }
...
${result.stderr.trim()}` : ""
      ...
      new import_obsidian2.Notice(status === "done" ? "知识库回答已完成" : stopped ? "知识库查询已停止" : "知识库查询失败");
    } catch (error) {
      ...
      new import_obsidian2.Notice(interrupted ? "知识库查询已停止" : `知识库查询失败：${message}`);
    } finally {
      this.activeRunId = "";
      this.activeMessageId = "";
      this.stopRequested = false;
  ...
  stopQuery() {
    ...
    if (!this.stopRequested) {
      new import_obsidian2.Notice("当前查询进程已经结束");
      return;
    }
...
${message.content}`).join("\n\n");
  ...
  async executeSynthesisHandoff(action, input, overrides) {
    if (this.plugin.isActionRunning(action.id)) {
      new import_obsidian2.Notice("综合分析正在运行");
      return;
    }
...
${result.stderr.trim()}` : ""
      ...
      new import_obsidian2.Notice(status === "done" ? "查询对话已整理为知识任务" : "整理为笔记失败");
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      completedRun = await this.plugin.finishTaskRun(run.id, {
        status: "failed",
      ...
      new import_obsidian2.Notice(`整理为笔记失败：${message}`);
    }
...
var DashboardView = class extends import_obsidian2.ItemView {
  ...
  registerVaultRefreshEvents() {
    ...
    this.registerEvent(this.app.vault.on("rename", (file, oldPath) => {
      ...
      if (this.isDashboardWikiPath(oldPath)) {
        this.pendingVaultChanges.set((0, import_obsidian2.normalizePath)(oldPath), {
          type: "delete",
          path: oldPath
        });
        queuedOldPath = true;
  ...
  isDashboardWikiPath(value) {
    const normalized = (0, import_obsidian2.normalizePath)(String(value || ""));
    return normalized.startsWith("wiki/") && normalized.toLowerCase().endsWith(".md");
  }
  queueVaultChange(type, file) {
    const filePath = (0, import_obsidian2.normalizePath)(String(file?.path || ""));
  ...
  requestStopRun(run) {
    ...
    if (!requested) {
      new import_obsidian2.Notice("任务进程已经结束，正在刷新运行状态");
      void this.loadAndRender();
      return;
    }
    this.stoppingRunIds.add(run.id);
    new import_obsidian2.Notice(`正在停止：${run.label}`);
    this.renderDashboard();
  }
  renderStats(parent) {
  ...
  renderKnowledgeGapAction(parent, gap) {
    ...
    (0, import_obsidian2.setIcon)(button.createSpan({ cls: "agent-dashboard-gap-action-icon" }), action?.id === "okf-export" ? "package-open" : "arrow-right");
    button.createSpan({ text: action && this.plugin.isActionRunning(action.id) ? "处理中" : "处理" });
    this.registerDomEvent(button, "click", () => {
      if (!action) {
        new import_obsidian2.Notice("该知识缺口尚未配置对应操作");
        return;
      }
  ...
  openAction(action, options = {}) {
    if (!action.enabled) {
      new import_obsidian2.Notice(`${action.label}将在后续阶段接入`);
      return;
    }
    if (action.queryView) {
    ...
    if (this.plugin.isActionRunning(action.id)) {
      new import_obsidian2.Notice(`${action.label}正在运行`);
      return;
    }
    if (action.localView) {
  ...
  async executeAction(action, input, executionOverrides = {}) {
    ...
    try {
      ...
      new import_obsidian2.Notice(status === "done" ? completionMessage : status === "interrupted" ? `${action.label}已停止` : `${action.label}执行失败`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      completedRun = await this.plugin.finishTaskRun(run.id, {
        status: "failed",
      ...
      new import_obsidian2.Notice(`${action.label}执行失败：${message}`);
    }
    await this.loadAndRender();
    if (completedRun) {
...
var AgentDashboardSettingTab = class extends import_obsidian2.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("项目根目录").setDesc("包含 AGENTS.md、.codex/ 和 tool-library/ 的项目目录。").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Codex 可执行文件").setDesc("用于文献、代码、检索和综合任务。默认自动选择当前 Codex 应用携带的最新 CLI；手动填写的外部路径会保留。").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Python 可执行文件").setDesc("用于统一 runner、知识库体检和 Python 代码练习。").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Rscript 可执行文件").setDesc("用于无状态 R 代码练习；不会自动安装 R 或 R 包。").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("代码练习超时（秒）").setDesc("每次 Python/R 练习的最长运行时间，范围 1-120 秒。").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("全局默认模型").setDesc("没有按钮级模型配置的 Dashboard AI 任务使用该模型。").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("全局默认推理强度").setDesc("仅在按钮没有指定推理强度时使用；按钮默认值和本次运行覆盖优先。").addDropdown((dropdown) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("任务超时（分钟）").setDesc("单个本地脚本或 Codex 任务的最长运行时间，范围 1-240 分钟。").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("运行环境").setDesc("检查项目根目录、Codex、Python 和 dashboard runner 是否可用。").addButton(
  ...
  renderProviderSettings(containerEl) {
    ...
    new import_obsidian2.Setting(containerEl).setName("当前执行后端").setDesc("Codex CLI：认证、模型调用、沙箱和权限继续由 Codex 管理。").addButton((button) => {
    ...
    const profileSetting = new import_obsidian2.Setting(containerEl).setName("配置").setDesc(profiles.length ? "切换当前编辑的供应商配置。" : "尚未创建 Direct API 配置。");
    ...
    if (!this.app.secretStorage || typeof import_obsidian2.SecretComponent !== "function") {
    ...
    if (!selectedProfile) {
      ...
      (0, import_obsidian2.setIcon)(icon, "plug-zap");
  ...
  renderProviderProfile(containerEl, profile) {
    ...
    new import_obsidian2.Setting(section).setName("配置名称").setDesc("用于区分多个供应商或不同账户。").addText((text) => {
    ...
    new import_obsidian2.Setting(section).setName("LLM Provider").setDesc("选择预定义供应商或 OpenAI 兼容服务。").addDropdown((dropdown) => {
    ...
    const secretSetting = new import_obsidian2.Setting(section).setName("API Key / 凭据").setDesc(
    ...
    if (this.app.secretStorage && typeof import_obsidian2.SecretComponent === "function") {
      secretSetting.addComponent(
        (element) => new import_obsidian2.SecretComponent(this.app, element).setValue(profile.secretId).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(section).setName("API Base URL").setDesc(`服务根地址。${metadata.defaultBaseUrl ? `默认：${metadata.defaultBaseUrl}` : ""}`).addText(
    ...
    const timeoutSetting = new import_obsidian2.Setting(section).setName("请求超时").setDesc(`模型发现和连接测试的单次请求上限。当前：${profile.timeoutSeconds} 秒。`).addSlider(
    ...
    const discoverySetting = new import_obsidian2.Setting(modelForm).setName("获取可用模型").setDesc("从当前 endpoint 获取最新模型列表，不发送 Vault 内容。");
    discoverySetting.addButton((button) => {
      ...
      button.setButtonText(loading ? "获取中…" : "获取模型列表").setCta().setDisabled(loading || runtime.status === "testing").onClick(async () => {
        ...
        try {
          ...
          new import_obsidian2.Notice(`已获取 ${models.length} 个模型`, 5e3);
        } catch (error) {
          const normalized = this.plugin.normalizeProviderError(error);
    ...
    const modelSetting = new import_obsidian2.Setting(modelForm).setName("选择模型").setDesc(discoveredModels.length ? `从 ${discoveredModels.length} 个可用模型中选择，也可手动填写模型 ID。` : "尚未获取模型列表，可先手动填写模型 ID。").addText(
    ...
    new import_obsidian2.Setting(modelForm).setName("模型能力").setDesc(
    ...
    new import_obsidian2.Setting(modelForm).setName("视觉输入").setDesc(
    ...
    new import_obsidian2.Setting(modelForm).setName("Qwen3.7-Plus 联网搜索").setDesc(
    ...
    if (qwenWebSearchAvailable && profile.webSearch.enabled) {
      new import_obsidian2.Setting(modelForm).setName("搜索协议").setDesc("当前 Direct API 适配器使用 /v1/chat/completions；不会发送 Responses API 或 DashScope 原生请求。").addDropdown(
      ...
      new import_obsidian2.Setting(modelForm).setName("强制联网").setDesc("启用 forced_search，确保选择“联网搜索”时每轮都触发搜索，而不是由模型自行判断。").addToggle(
      ...
      const webTimeoutSetting = new import_obsidian2.Setting(modelForm).setName("联网请求超时").setDesc(
      ...
      new import_obsidian2.Setting(modelForm).setName("搜索策略").setDesc("turbo 适合日常查询；max 搜索更全面；agent 可能受模型版本和地域限制。").addDropdown(
      ...
      new import_obsidian2.Setting(modelForm).setName("限定搜索站点").setDesc(
    ...
    const controls = new import_obsidian2.Setting(modelForm).setName("测试连接").setDesc("验证 endpoint、凭据、模型和流式协议；成功后自动设为默认 Direct API 配置。");
  ...
  renderConnectionResult(parent, result) {
    ...
    (0, import_obsidian2.setIcon)(icon, result.ok ? "circle-check" : "circle-alert");
...
var AgentDashboardPlugin = class extends import_obsidian2.Plugin {
  async onload() {
  ...
  recoverInterruptedPracticeRuns() {
    const runsDirectory = path2.join(this.settings.projectRoot, "tool-library", "output", "code-practice", "runs");
...
Execution interrupted before the plugin restarted.`.trim();
  ...
  runCodePractice(request) {
    ...
    const stopPath = path2.join(projectRoot, "tool-library", "output", "code-practice", "stop", `${request.run_id}.stop`);
    const args = [
      runner,
      "--project-root",
      projectRoot,
  ...
  stopCodePractice(runId) {
    ...
    try {
      fs.mkdirSync(path2.dirname(active.stopPath), { recursive: true });
  ...
  readPracticeFigure(relativePath) {
    const root = path2.resolve(this.settings.projectRoot);
    const outputRoot = path2.join(root, "tool-library", "output", "code-practice", "figures");
    const candidate = path2.resolve(root, relativePath);
    const relative2 = path2.relative(outputRoot, candidate);
    if (!relative2 || relative2.startsWith("..") || path2.isAbsolute(relative2) || !fs.existsSync(candidate)) return "";
    const stat = fs.statSync(candidate);
    if (!stat.isFile() || stat.size > 10 * 1024 * 1024) return "";
    const mime = { ".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".svg": "image/svg+xml" }[path2.extname(candidate).toLowerCase()];
    if (!mime) return "";
    return `data:${mime};base64,${fs.readFileSync(candidate).toString("base64")}`;
  }
  async savePracticeNote(payload) {
    const folder = (0, import_obsidian2.normalizePath)("wiki/code/practice");
    ...
    let notePath = (0, import_obsidian2.normalizePath)(`${folder}/${slugBase || fallback}.md`);
    if (this.app.vault.getAbstractFileByPath(notePath)) {
      notePath = (0, import_obsidian2.normalizePath)(`${folder}/${slugBase || "practice"}-${lastResult.run_id?.slice(-6) || Date.now()}.md`);
    }
  ...
  async ensureVaultFolder(folderPath) {
    let current = "";
    for (const segment of (0, import_obsidian2.normalizePath)(folderPath).split("/")) {
  ...
  async loadSettings() {
    ...
    if (!configuredCodexExecutable || !fs.existsSync(configuredCodexExecutable) || isManagedCodexExecutable(configuredCodexExecutable)) {
  ...
  scheduleSettingsSave(delayMs = 400) {
    ...
    return new Promise((resolve2, reject) => {
      this.settingsSaveWaiters.push({ resolve: resolve2, reject });
  ...
  async flushScheduledSettingsSave() {
    ...
    try {
      await this.saveSettings();
      waiters.forEach(({ resolve: resolve2 }) => resolve2());
    } catch (error) {
  ...
  inferProjectRoot() {
    ...
    const parent = path2.dirname(vaultRoot);
    if (fs.existsSync(path2.join(parent, "AGENTS.md"))) return parent;
    return vaultRoot;
  }
  getTaskRuns() {
  ...
  async persistTaskRunOutput(run) {
    ...
    await fs.promises.mkdir(path2.dirname(absolutePath), { recursive: true });
  ...
  getOkfExportStatus() {
    const projectRoot = this.settings.projectRoot;
    const exporter = path2.join(projectRoot, "tool-library", "scripts", "export_okf.py");
    const latestPath = path2.join(projectRoot, "tool-library", "output", "okf", "latest.json");
    let latest = null;
    let error = "";
  ...
  getLintStatus() {
    const projectRoot = this.settings.projectRoot;
    const latestPath = path2.join(projectRoot, "tool-library", "output", "lint", "latest.json");
    let latest = null;
    let error = "";
    if (fs.existsSync(latestPath)) {
      try {
  ...
  checkRuntime(action = null) {
    ...
    const practiceRunner = path2.join(projectRoot, "tool-library", "scripts", "run_code_practice.py");
    const exporter = path2.join(projectRoot, "tool-library", "scripts", "export_okf.py");
    const lintScript = path2.join(projectRoot, "tool-library", "scripts", "lint_vault.py");
    const checks = [
      ["项目根目录", fs.existsSync(projectRoot)],
      ["AGENTS.md", fs.existsSync(path2.join(projectRoot, "AGENTS.md"))],
  ...
  buildDirectRetrievalResult(text, evidence, trace, mode, profile) {
    ...
    const vaultSources = (Array.isArray(evidence) ? evidence : []).filter((item) => {
      ...
      path: item.path,
      title: path2.posix.basename(item.path, ".md"),
      cited: true
    }));
  ...
  runVaultRetrievalPreflight(runId, question, expandedTerms = []) {
    const projectRoot = path2.resolve(this.settings.projectRoot);
    const script = path2.join(projectRoot, "tool-library", "scripts", "retrieve_vault.py");
  ...
  readVaultEvidencePacket(trace) {
    const projectRoot = path2.resolve(this.settings.projectRoot);
    const vaultRoot = path2.resolve(projectRoot, "knowledge-base");
    const vaultPrefix = `${vaultRoot}${path2.sep}`;
    ...
    for (const candidate of candidates) {
      ...
      const absolutePath = path2.resolve(vaultRoot, ...relativePath.split("/"));
  ...
  resolveVaultLinkedFile(rawLink, sourcePath = "") {
    ...
    link = (0, import_obsidian2.normalizePath)(link.replace(/^knowledge-base\//i, ""));
    if (!link) return null;
    ...
    if (sourcePath) {
      const relative2 = (0, import_obsidian2.normalizePath)(
        path2.posix.normalize(path2.posix.join(path2.posix.dirname(sourcePath), link))
      );
  ...
  resolveVaultMarkdownFile(rawLink) {
    ...
    candidate = (0, import_obsidian2.normalizePath)(candidate.replace(/^knowledge-base\//i, ""));
  ...
  extractQuestionNoteFiles(question) {
    ...
    for (const match of text.matchAll(/\[\[([^\]]+)\]\]/g)) {
      ...
      if (!VAULT_IMAGE_MIME_TYPES[path2.posix.extname(value).toLowerCase()]) {
  ...
  async getEmbeddedImageFiles(noteFile) {
    ...
    for (const link of links) {
      ...
      if (!file || !VAULT_IMAGE_MIME_TYPES[path2.posix.extname(file.path).toLowerCase()]) continue;
  ...
  buildVaultImageReferenceIndex(imageFiles = []) {
    const normalizeVaultPath = (value) => (0, import_obsidian2.normalizePath)(
    ...
    const addReference = (imagePathValue, notePathValue, countValue = 1) => {
      ...
      const title = String(
        frontmatter?.title_zh || frontmatter?.title || noteFile?.basename || path2.posix.basename(notePath, ".md")
      ).trim();
      ...
      referenceMaps.get(imagePath).set(notePath, {
        path: notePath,
        title: title || path2.posix.basename(notePath, ".md"),
  ...
  readVaultImageData(attachment) {
    ...
    const absolutePath = path2.resolve(vaultRoot, ...normalized.path.split("/"));
    ...
    if (normalizedImage !== normalizedVault && !normalizedImage.startsWith(`${normalizedVault}${path2.sep}`)) {
      throw new ProviderConnectionError("attachment", "图片路径超出当前 Vault");
    }
    ...
    const extension = path2.extname(imageRealPath).toLowerCase();
    const mimeType = VAULT_IMAGE_MIME_TYPES[extension];
    if (!mimeType) {
  ...
  runVaultAction(runId, action, input, executionConfig = null, hooks = {}) {
    ...
    fs.mkdirSync(path2.dirname(stopPath), { recursive: true });
