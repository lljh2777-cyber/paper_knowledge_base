...
assert.match(runner, /duplicate_scope: per-output-slot/);
assert.match(runner, /identity_match_does_not_satisfy_missing_outputs: yes/);
assert.match(runner, /knowledge-base\/Clippings\//);
assert.match(runner, /Continue creating every\s+missing requested slot/);
...
assert.match(agents, /Check requested outputs independently/);
assert.match(agents, /knowledge-base\/Clippings\//);
assert.match(agents, /Whole-action duplicate is valid only when all requested slots independently exist/);
