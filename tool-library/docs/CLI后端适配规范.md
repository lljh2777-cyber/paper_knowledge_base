...
发布包中的 `_extraction/viewer-index.json` 会保留 MinerU 图注数组的原子分类，
`visual-repair.json` 只记录确定性的碎图与跨页图注关系；阅读器可在不修改
`article.md` 的前提下，将同页图注原子链绑定到具体 Markdown 图片 occurrence；
仅当完整原子顺序能以唯一方式分布在图片前后、且中间至多有两个空白行时才隐藏，
因此可同时处理单字母面板标签、图内标题、正式图注与拆分续段。对于 MinerU 输出为
独立文本块的 `Extended Data Fig. x |` 等同页正式图注，识别范围也包括
`Supplementary`、`Supporting`/`Supporting Information`、小数图号，以及省略分隔符但
直接接完整标题的形式。标题以 `shows`、`illustrates`、`provides` 等正文引用动词开头，
或标题过短时不算正式图注。阅读器先按 MinerU 从上到下、从左到右的源序，为每个图块
选择其后第一个正式图注，再要求唯一空间近邻；共享同一后继图注且坐标相邻的碎图组件
可合并，跨越不同图注边界或仅命中正文引用的组件必须分开保留；
若正式图注因双栏排版拆成两块，只有第二块与首块处于同一图注带、以小写或面板标签
连续、并以终止标点闭合时，才会把两块作为一个原子链隐藏，禁止只隐藏首块留下孤立尾段。
若它唯一锚定一条由窄间隔、带面板标签的横向视觉带，则在运行时合并已验证的 PDF
裁剪组。若一个自动重建裁剪框覆盖较小裁剪框至少 97%、面积至少为其 1.35 倍，且两组
在源序中相邻并只能解析出同一个正式图号，则把较小的 p/q/r 式重复子图折叠进完整大图；
其成员、Markdown 图片 occurrence 和图注锚点一并转移，因此跨页图注仍绑定到完整大图。
只有部分重叠、没有正式图号或图号冲突时必须分别保留。若完整视觉由带 `asset_path` 的 HTML `<table>` 表示且没有 Markdown 图片 ID，
只有表格正文与正式图注均能唯一定位、同页空间归属唯一、源序紧邻且二者之间仅为空白时，
才会在保持表内图片 occurrence 计数后原子替换整张表与图注。重复、改写或正文插入时全部保留。
两个并排组件各有图注或归属同样合理时必须保守保留。跨页图注还必须由目标文本块携带并通过
精确 Markdown 字符区间校验。完整目标页必须通过验证，区间还要落在源序与 Markdown
顺序单调一致的前后图片 occurrence 之间，并绑定连续的目标页文本块链，或绑定来源
图片旁已验证的跨页占位原子。页首短标题只有在其他页存在文本完全相同、坐标近似且
明确标为 `header`/`page_header` 的对应块时，才按运行页眉跳过并允许严格运行时补链；
渲染层还要求该页眉以唯一的 Markdown ATX 标题形式落在两个源序一致的视觉边界之间，
且每个边界必须能验证为 Markdown 图片 occurrence 或唯一的独立 HTML `<table>` 区间，
才隐藏这一处；缺少边界或边界内出现重名标题时全部保留，禁止按全文
同名字符串猜测删除。任何非空断行、乱序、
区间失效或正文碰撞都保守保留。
`visual-candidates.json` 只为 `review`、`partial` 或 `ambiguous`
情况生成哈希绑定候选，不会自动调用外部模型。后续 AI 裁决只能返回已有候选 ID
以及 `accept`、`reject` 或 `abstain`，不得生成坐标、路径或图注正文；无效裁决
必须回退到确定性结果或 MinerU 原图。
