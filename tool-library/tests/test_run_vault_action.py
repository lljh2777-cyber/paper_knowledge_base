...
class RunVaultActionQuerySessionTests(unittest.TestCase):
    ...
    def test_paper_ingest_options_are_normalized_as_authoritative(self) -> None:
        ...
        self.assertIn("duplicate_scope: per-output-class", request)
        self.assertIn(
            "original_markdown_uniqueness_roots: knowledge-base/papers, knowledge-base/Clippings",
            request,
        )
        self.assertIn("analysis_uniqueness_root: knowledge-base/wiki", request)
        self.assertIn(
    ...
    def test_paper_ingest_prompt_keeps_originals_and_analysis_independent(self) -> None:
        prompt = run_vault_action.build_prompt(
            "paper-ingest",
        ...
        self.assertIn("Evaluate two output classes independently", prompt)
        self.assertIn("one shared\nuniqueness domain", prompt)
        ...
        self.assertRegex(
            prompt,
            r"Continue creating every missing requested class",
        )
        self.assertIn("every requested class", prompt)
        self.assertIn("Before\nrunning MinerU, check both roots", prompt)
