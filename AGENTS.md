...
- Treat document identity and output-class existence as separate checks. A DOI, normalized-title, source-path, or PDF-hash match resolves the existing identity/citekey; it does not prove that every requested output class already exists.
- Check the two output classes independently and report one status per requested class:
  - Original Markdown is one shared uniqueness domain spanning `knowledge-base/papers/` and `knowledge-base/Clippings/`. A complete identity match in either root satisfies the original-Markdown class, so do not generate the same document again in the other root. Apply the integrity rules appropriate to the located form: validate a structured MinerU package under `papers/`, or verify the exact faithful Markdown file under `Clippings/`.
  - Analysis/authored knowledge is a separate uniqueness domain under `knowledge-base/wiki/`. A matching original under `papers/` or `Clippings/` does not make a requested Wiki analysis duplicate, and an existing Wiki note does not satisfy a requested original-Markdown output.
- Never return a whole-action duplicate/skip merely because one output class already contains the document. Reuse or skip only the satisfied requested class, and continue producing every other requested class that is absent. Whole-action duplicate is valid only when all requested classes independently exist and pass their own validation.
